...
//		LeftHandCursor,
        ...
        /// <summary>
        /// 双手下垂斜摆放，且手，手肘，肩膀连成一条直线，摆放角度跟水平成60°左右夹角
        /// </summary>
        ObliqueHand,
...
//							}
			...
			case Gestures.Run:
				...
				break;
            case Gestures.ObliqueHand:
                switch (gestureData.state)
                {
                    case 0: // gesture detection  判断肩膀到手肘的向量，手肘到手的向量的夹角小于某个角度就算三点在一条直线上  

                        bool isExactness = CheckPose(jointsTracked, jointsPos);
                        if (isExactness)
                        {
                            SetGestureJoint(ref gestureData, timestamp, rightHandIndex, jointsPos[rightHandIndex]);
                        }
                    ...
                    case 1:  // gesture complete
                        bool isInPose = CheckPose(jointsTracked, jointsPos);

                        Vector3 jointPos = jointsPos[gestureData.joint];
                        CheckPoseComplete(ref gestureData, timestamp, jointPos, isInPose, KinectInterop.Constants.PoseCompleteDuration);

                        //Debug.Log("state is " + gestureData.state);
                        break;
                }
                break;
	...
	private GameObject rightElbowGameObject;

	private GameObject rightShoulderGameObject;
	/// <summary>
	/// 拓展算法，加测躯体，双手是否是斜直线摆放，并且成一定的角度
	/// </summary>
	/// <param name="jointsTracked"></param>
	/// <param name="jointsPos"></param>
	/// <returns></returns>
	private bool CheckPose(bool[] jointsTracked, Vector3[] jointsPos, float accordAngel = 0f)
	{
		bool flag = this.rightHandGameObject == null;
		if (flag)
		{
			this.rightHandGameObject = new GameObject();
			this.rightHandGameObject.name = "rightHandGameObject";
			this.rightHandGameObject.transform.localScale = Vector3.one * 0.15f;
		}
		bool flag2 = this.rightElbowGameObject == null;
		if (flag2)
		{
			this.rightElbowGameObject = new GameObject();
			this.rightElbowGameObject.name = "rightElbowGameObject";
			this.rightElbowGameObject.transform.localScale = Vector3.one * 0.15f;
		}
		bool flag3 = this.rightShoulderGameObject == null;
		if (flag3)
		{
			this.rightShoulderGameObject = new GameObject();
			this.rightShoulderGameObject.name = "rightShoulderGameObject";
			this.rightShoulderGameObject.transform.localScale = Vector3.one * 0.15f;
		}
		bool isRightExactness = false;
		bool flag4 = jointsTracked[this.rightHandIndex] && jointsTracked[this.rightElbowIndex] && jointsTracked[this.rightShoulderIndex];
		if (flag4)
		{
			this.rightShoulderGameObject.transform.position = jointsPos[this.rightShoulderIndex];
			this.rightElbowGameObject.transform.position = jointsPos[this.rightElbowIndex];
			this.rightHandGameObject.transform.position = jointsPos[this.rightHandIndex];
			Vector3 dir = (jointsPos[this.rightHandIndex] - jointsPos[this.rightElbowIndex]).normalized;
			Debug.DrawLine(jointsPos[this.rightElbowIndex], jointsPos[this.rightElbowIndex] + dir, Color.red);
			Vector3 dir2 = (jointsPos[this.rightElbowIndex] - jointsPos[this.rightShoulderIndex]).normalized;
			Debug.DrawLine(jointsPos[this.rightShoulderIndex], jointsPos[this.rightShoulderIndex] + dir2, Color.green);
			float angle = Vector3.Dot(dir2, dir);
			bool flag5 = angle > 0.9f;
			if (flag5)
			{
				isRightExactness = true;
			}
			bool flag6 = isRightExactness;
			if (flag6)
			{
		...
		bool isLeftExactness = false;
		bool flag8 = jointsTracked[this.leftHandIndex] && jointsTracked[this.leftElbowIndex] && jointsTracked[this.leftShoulderIndex];
		if (flag8)
		{
			Vector3 dir4 = (jointsPos[this.leftHandIndex] - jointsPos[this.leftElbowIndex]).normalized;
			Vector3 dir5 = (jointsPos[this.leftElbowIndex] - jointsPos[this.leftShoulderIndex]).normalized;
			float angle3 = Vector3.Dot(dir5, dir4);
			bool flag9 = angle3 > 0.9f;
			if (flag9)
			{
				isLeftExactness = true;
			}
			bool flag10 = isLeftExactness;
			if (flag10)
			{
		...
		return isLeftExactness && isRightExactness;
	}
