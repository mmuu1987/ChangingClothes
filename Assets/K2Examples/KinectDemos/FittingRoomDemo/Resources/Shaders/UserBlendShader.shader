	...
	SubShader 
	{
		...
		Pass
		{
			...
			bool CheckHead(float2 uv)
			{
			   ...
			   float2 newUV = float2((uv.x -_ColorOfsX)/_ColorMulX,((uv.y-_ColorOfsY)/_ColorMulY)*1.25);
			...
			bool CheckHeadUp(float2 uv)
			{
				...
				if (_HeadRadius- _WidthBule <= dis)//缩小的范围
				{
					
					//跟Up轴垂直的角度
					float2 dir = newUV - _HeadScrPos.xy;
					dir = normalize(dir);
					float angle = dot(float2(0, 1), dir);

					if (angle <= _Angle)//说明在夹角内
					{
						return true;
					}
				}
					...
					//if(di >= 0 && di < di_length)
					{
						...
						else
						{
							...
							else//头部里面
							{
								...
								if (clrFront.x + clrFront.y + clrFront.z >= _White)clrFront.a = 0;//背景优化，因为这个项目的背景是白墙居多

								if (clrFront.a <= _Alpha)clrFront = mainTex;

								/*bool isUp = CheckHeadUp(ctUv);
