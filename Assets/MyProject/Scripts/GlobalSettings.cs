...
public static class GlobalSettings
{
										...
										else
										{
											...
											if (flag11)
											{
												foreach (object obj2 in item.ChildNodes)
												{
													...
													rif.BodyScale = float.Parse(element.GetAttribute("BodyScale"));
													rif.ArmScale = float.Parse(element.GetAttribute("ArmScale"));
													rif.LegScale = float.Parse(element.GetAttribute("LegScale"));
													rif.Vertacal = float.Parse(element.GetAttribute("Vertacal"));
													rif.Farward = float.Parse(element.GetAttribute("Farward"));
													rif.HatHeight = float.Parse(element.GetAttribute("HatHeight"));
													rif.Name = element.GetAttribute("Name");
													rif.id = (float)int.Parse(element.GetAttribute("id"));
													rif.HeadScale = float.Parse(element.GetAttribute("HeadScale"));
													rif.Shifting.x = float.Parse(element.GetAttribute("ShiftingX"));
													rif.Shifting.y = float.Parse(element.GetAttribute("ShiftingY"));
													rif.Shifting.z = float.Parse(element.GetAttribute("ShiftingZ"));
													GlobalSettings.RoleInfos.Add(rif);
												}
											}
										}
									}
	...
	public static void SaveXml()
	{
		...
		int i = 0;
		foreach (RoleInfo roleInfo in GlobalSettings.RoleInfos)
		{
			i++;
			XmlElement temp6 = xml.CreateElement("Role" + i);
			temp6.SetAttribute("id", roleInfo.id.ToString(CultureInfo.InvariantCulture));
			temp6.SetAttribute("Name", roleInfo.Name.ToString(CultureInfo.InvariantCulture));
			temp6.SetAttribute("BodyScale", roleInfo.BodyScale.ToString(CultureInfo.InvariantCulture));
			temp6.SetAttribute("ArmScale", roleInfo.ArmScale.ToString(CultureInfo.InvariantCulture));
			temp6.SetAttribute("LegScale", roleInfo.LegScale.ToString(CultureInfo.InvariantCulture));
			temp6.SetAttribute("Vertacal", roleInfo.Vertacal.ToString(CultureInfo.InvariantCulture));
			temp6.SetAttribute("Farward", roleInfo.Farward.ToString(CultureInfo.InvariantCulture));
			temp6.SetAttribute("HatHeight", roleInfo.HatHeight.ToString(CultureInfo.InvariantCulture));
			temp6.SetAttribute("HeadScale", roleInfo.HeadScale.ToString(CultureInfo.InvariantCulture));
			temp6.SetAttribute("ShiftingX", roleInfo.Shifting.x.ToString(CultureInfo.InvariantCulture));
			temp6.SetAttribute("ShiftingY", roleInfo.Shifting.y.ToString(CultureInfo.InvariantCulture));
			temp6.SetAttribute("ShiftingZ", roleInfo.Shifting.z.ToString(CultureInfo.InvariantCulture));
			roleInfos.AppendChild(temp6);
		}
	...
	public static RoleInfo GetRoleInfo(string name)
	{
		foreach (RoleInfo roleInfo in GlobalSettings.RoleInfos)
		{
			bool flag = roleInfo.Name == name;
			if (flag)
			{
				return roleInfo;
			}
	...
	public static void SaveRoleInfo(RoleInfo info)
	{
		foreach (RoleInfo roleInfo in GlobalSettings.RoleInfos)
		{
			bool flag = info.Name == roleInfo.Name;
			if (flag)
			{
				roleInfo.BodyScale = info.BodyScale;
				roleInfo.ArmScale = info.ArmScale;
				roleInfo.LegScale = info.LegScale;
				roleInfo.Vertacal = info.Vertacal;
				roleInfo.Farward = info.Farward;
				roleInfo.HatHeight = info.HatHeight;
	...
	public static List<RoleInfo> RoleInfos = new List<RoleInfo>();
