...
Shader "MyShader/UserBlendShader_Shadow_UnTransparent"
{
	Properties
	{
				...
				if (!isinf(_DepthCoords[ci].x) && !isinf(_DepthCoords[ci].y))
				{
					...
					//if(di >= 0 && di < di_length)
					{
							...
							camDepth <= (kinDepth + _Threshold))
						{
						   ...
						   if (col.x + col.y + col.z >= 3)//如果是地板纯白色，直接换成背景颜色
							{
							 col = tex2D(_BackTex, ctUv2);
							}
						return col;
						}
						else
						{
							...
							if (value)//如果满足条件，说明是在头部范围外面
							{
								...
								if (mainTex.a + mainTex.y + mainTex.z >= 3)//如果是地板标志，地板材质颜色为纯白色，把它换成背景颜色
								{
									clrFront = clrBack;
								}
								}
